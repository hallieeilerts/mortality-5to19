fn_randAssignMeas <- function(CSMFDRAW){
  ...
  dat$meas_all_ub[which(dat$meas_all_ub > dat$Deaths2)] <- dat$Deaths2[which(dat$meas_all_ub > dat$Deaths2)]
  ...
  if(length(v_idSqz) > 0){
    ...
    v_msl[v_idSqz] <- fn_sampleLogNorm(MU = dat$meas_all[v_idSqz],
                                         LB = dat$meas_all_lb[v_idSqz],
                                         UB = dat$meas_all_ub[v_idSqz])
    ...
    # Note: Measles and meas_epi do not need to add up to meas_all, but their total should be within meas_all_lb and meas_all_ub
