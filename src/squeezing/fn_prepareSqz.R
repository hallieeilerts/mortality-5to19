fn_prepareSqz <- function(CSMF, DAT_TB, DAT_HIV, DAT_CRISIS, DAT_MEAS, FRAC_CD, FRAC_LRI){
  ...
  # Merge on TB
  dat <- merge(CSMF, DAT_TB, by = c("iso3", "year"), all.x = T)
  
  # Merge on HIV
  dat <- merge(dat, DAT_HIV, by = c("iso3", "year"), all.x = T)
  
  # Merge on measles
  if(!is.null(DAT_MEAS)){
    dat <- merge(dat, DAT_MEAS, by = c("iso3", "year"), all.x = T)
  }
  
  # Merge on crisis
  dat <- merge(dat, DAT_CRISIS, by = c("iso3", "year"), all.x = T)
